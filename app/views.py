...
class HomePageView(ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        context['local_news'] = New.published.filter(category__name='Mahalliy')[:5]
        context['sport_news'] = New.published.filter(category__name='Sport')[:5]
        context['technology_news'] = New.published.filter(category__name='Texnologiya')[:5]
        context['foreign_news'] = New.published.filter(category__name='Xorij')[:5]
        context['ommabop_news'] = New.published.all().order_by('-publish_time')[:5]
        return context
...
def news_detail(request, slug):
    new = get_object_or_404(New, slug=slug, status=New.Status.PUBLISHED)
    context = {
        'new': new,
    }
